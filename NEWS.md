...
* `read_cbs_index()` now detects the geographic level of the file (municipality, locality or statistical area) from its content and applies the matching header layout, so it no longer matters which table number the CBS used for that level in a given edition; the detected level is also stored in the `"unit_type"` attribute of the result.
* `read_cbs_index()` deprecates the `unit_type` argument, which is now ignored because the level is detected automatically.
